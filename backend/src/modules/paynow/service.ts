...
import { paynowClient as paynow } from "./client";
...
class PaynowProviderService extends AbstractPaymentProvider<Options> {
  ...
  async authorizePayment(
    ...
    const payload = {
      title: "Option One",
      lineTotal: parseFloat(paymentSessionData.amount as string) / 100,
    } as { title: string; lineTotal: any };

    const { phone, operator, origin }: any = paymentSessionData.session_data;
    console.table({ phone, operator, lineTotal: payload.lineTotal });
    const res = await paynow(payload, phone, operator, origin);
  ...
  async initiatePayment(
    ...
    console.log("[initiatePayment] context: ", context);
